...
class ADNIDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 classes: list,
                 class_to_idx: dict,
                 data_augmentation: RandomTransformations | None = None,   
                 data_augmentation_slice: transforms.Compose | None = None,
                 num_slices: int = 80,
    ...
    def load_image(self, index: int):
        ...
        return nibabel.loadsave.load(image_path)
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        if resize_shape is not None:
            if isinstance(resize_shape, int):
                transformed_mri = torch.empty((self.num_slices, 3, resize_shape, resize_shape))
            else:
